...
pattern = r"(?=(XMAS)|(SAMX))"
# J'ai du ajouter le ?= car sinon re.findall() ne trouvait pas les overlappings, par exemple dans XMASAMX
...
def diag(data):
    ...
    for i in range(ni - 1):
        ...
        for j in range(nj):
            # Maximum nj charactères avant d'arriver à la droite de data
            if i + 1 + j < ni:
                result[i] += data[i + 1 + j][j]
...
def diag_reverse(data):
    ...
    for i in range(ni - 1):
        ...
        for j in range(nj):  # Maximum nj charactères avant d'arriver à gauche de data
            if i + 1 + j < ni and j < ni:
                result[i] += data[i + 1 + j][nj - 1 - j]
    # On remplit les nj autres lignes (diagonales dont les premiers charactères sont ceux de la première ligne de data)
    for i in range(nj):
        # Pour chaque ligne, on parcout la diagonale jusqu'à ce qu'on arrive en bas ou à droite de data
        for j in range(nj):  # idem, maximum nj caractères
            if -i + nj - 1 - j >= 0 and j < ni:
                result[ni - 1 + i] += data[j][-i + nj - 1 - j]
    return result
...
# Une autre solution vue sur reddit était de faire des "minitables" de 4x4, on a ensuite plus facilement accès aux diagonales dans cette minitable
# il y a aussi des solutions avec des énumérations de directions
# il y a aussi des gens qui ont fait leurs propres outils en amont pour gérer les diagonales
